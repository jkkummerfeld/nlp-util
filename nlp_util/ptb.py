...
def counts_for_prf(test, gold):
	...
	for error in errors:
		if error[0] == 'extra':
			...
			gmatch -= 1
###	assert tmatch == gmatch, "tmatch != gmatch, (%d %d) from (%d %d %d %d %d) \nTest: %s\n%s\n\nGold: %s\n%s" % (tmatch, gmatch, test_count, len(errors.extra), gold_count, len(errors.missing), len(errors.crossing), test.__repr__(), test.colour_repr(gold, single_line=True), gold.__repr__(), gold.colour_repr(test, single_line=True))
	return tmatch, gold_count, test_count

# TODOtodo: create function that iterates through sets of trees, calling a given function with each tree n-tuple
