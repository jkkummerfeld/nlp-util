...
def amr_format(tree, used=None, depth=0):
	if used is None:
		used = set()
	ans = ''
	variable = tree.label[0].lower()
	if variable in used:
		num = 1
		while "{}{}".format(variable, num) in used:
			num += 1
		variable = "{}{}".format(variable, num)
	used.add(variable)
	if depth == 0:
		ans += "# ::snt {}\n# ::ptb {}".format(text_words(tree), text_tree(tree))
	ans += '\n{}'.format(depth * 4 * ' ')
	if tree.word is None:
		if depth > 0:
			ans += ':{} '.format(tree.label.lower())
		ans += '({} / {}'.format(variable, tree.label.lower())
		for subtree in tree.subtrees:
			ans += ' ' + amr_format(subtree, used, depth + 1)
		ans += ')'
	else:
		if tree.label.lower() == tree.label.upper():
			ans += ':symbol "{}"'.format(tree.word)
		else:
			ans += ':{} "{}"'.format(tree.label.lower(), tree.word)
	if depth == 0:
		ans += '\n'
	return ans
